...
public class MAINGUI2 extends PFrame implements ActionListener
{
    ...
    int[] profilelookup;
    int[] ranklookup;
    static int[] cluster;  //cluster each profile belongs to 
    ...
    boolean ClusterFlag = false; //false - use model profiles in comparison/ true- use clusters in comparison
    ...
    JButton CompareClusterHButton = new JButton(Util.createImageIcon("Help16.gif"));
    ...
    JCheckBox CompareClusterCheck = new JCheckBox("Compare Clusters", false);
    ...
    /**
     * Automatically run comparison under auto mode (-a option in command line)
    */
    public void AutoCompare(boolean clusterflag)
    {
           String szcompare1val =  ST.szCompareDEF;          
           String szsigLevel = String.valueOf(ST.dCompareMinpvalDEF);
           String sznumgenes = String.valueOf(ST.nCompareMinGenesDEF);
	       ...
	       final CompareInfo ci = new CompareInfo(theDataSet, szcompare1val,
			 szsigLevel, sznumgenes, vRepeatFilesDEF, ballselected,cf,ST.ClusterFlag);
               ...
               javax.swing.SwingUtilities.invokeLater(new Runnable() 
               {
                  public void run() 
                  {
                     try
                     {
		        thecomparegui = new CompareGui(ci,profilenodes,thegeneplotpanel,cf,ClusterFlag);
                           ...
                           if (pt.defineframe != null)
			   {
		        ...
		        //thecomparegui.setVisible(false);
               	//thecomparegui.dispose();
		     }
                     catch (Exception ex)
                     {
		           ...
		           if ((theDataSet.pmavalues[nrow][ncol]>0)&&(theDataSet.pmavalues[nrow][ncol+1]>0))
	                   {
	                       ...
	                       double deltaLine = ddeltax; //*4/(theDataSet.numcols);
			       xp[0] = (float) (deltaLine*theDataSet.dwidthunitsCum[ncol]*(theDataSet.data[0].length-1)); 
                              xp[1] = (float) (deltaLine*theDataSet.dwidthunitsCum[ncol+1]*(theDataSet.data[0].length-1));
                              yp[0] = (float) (deltaLine/2*
		  		   (theDataSet.numcols-1-dfactor*
		  		    dgenek*theDataSet.data[nrow][ncol]));
                              yp[1] = (float) (deltaLine/2*
		  		      (theDataSet.numcols-1-dfactor*
	...
	else if (esource == CompareClusterHButton)
	{
            szMessage = "Compare clusters if checked. Otherwise, compare model profiles";
	}
        ...
        else if (esource == compareButtonDialog)
	{
	   String szcompare1val =  compare1Field.getText();          
           ...
           if (CompareClusterCheck.isSelected() )
	       {
	       //synchronized (lockpd)
               //{
	           ClusterFlag = true;
	    ...
	    else
	    {
	       ...
	       boolean ballselected = theCompareRepeatList.allButton.isSelected();
	       final CompareInfo ci = new CompareInfo(theDataSet, szcompare1val,
			 szsigLevel, sznumgenes, theCompareRepeatList.data, ballselected,cf,ClusterFlag);
    ...
    protected void makeComparePanel(Container cp)
    {
       ...
       SpringUtilities.makeCompactGrid(prepeat,2,2,6,6,6,6);    // rows, cols, initX, initY, xPad, yPad    
       cp.add(prepeat);
       ...
       JPanel comparepanel = new JPanel(new SpringLayout());
       ...
       CompareClusterHButton.addActionListener(this);
       
       CompareClusterCheck.setBackground(ST.lightBlue);
       CompareClusterCheck.addActionListener(this);
       CompareClusterCheck.setPreferredSize(new Dimension(250,30));
       CompareClusterCheck.setMinimumSize(new Dimension(250,30));
       CompareClusterCheck.setMaximumSize(new Dimension(300,30));
       
       comparepanel.add(CompareClusterCheck);
       comparepanel.add(CompareClusterHButton);
       comparepanel.add(compareButtonDialog);
       comparepanel.add(executecompareHButton);
       
       if (ST.ClusterFlag)
       {
           //System.out.println("ST.ClusterFlag is true");
           CompareClusterCheck.setSelected(true);
       }

       SpringUtilities.makeCompactGrid(comparepanel,2,2,6,6,6,6);
       cp.add(comparepanel);
    }
