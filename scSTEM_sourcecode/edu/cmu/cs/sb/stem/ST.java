...
public class ST extends JFrame implements ActionListener
{
    ...
    static int autoflag = 0; // 1: auto mode, run clustering and comparison if files are specified
    static boolean ClusterFlag = false; 
    ...
    */
	...
	infoButton.setBackground(ST.lightBlue);
        ...
        // handling auto run
        if (!szDataFileDEF.equals("") && (autoflag == 1) )
        {
            try
	    {
                s1 = System.currentTimeMillis();
                String szcommand = "Execute";
            ...
            {
                System.out.println("Something went wrong when clustering.");
            }
        }
			    ...
			    (sztype.equalsIgnoreCase("Y_Scale_Min")))
		   {
		    ...
		    else if (sztype.equalsIgnoreCase("Gene_Location_File"))
		    {
		       szChromFileDEF = szvalue;
	            ...
	            {
	                if (szDataFileDEF.equals(""))   // could be specified in command line
	                {
	  	    ...
	  	    {
	  	        if (szCompareDEF.equals(""))   // could be specified in command line
	                {
           ...
           else
	   {
	      ...
	      if (thefDataSetfmnel.numcols >= 2)
	      {
			  ...
			  StatUtil.htbinom = new Hashtable();
                	...
                	if ((autoflag == 1) && (!szCompareDEF.equals("")) )
  	               {
  	                 System.out.println("in auto compare");
  	                 f.AutoCompare(ClusterFlag);
  	               }
   ...
   public class Progressdownload implements Runnable
   {
        ...
        if (args.length == 0)
	{
        ...
        {
            if (args[i].equals("-d"))    // -d: default settings
            {
                if (i+1 < args.length)
                {
                    szDefaultFile = args[i+1];
                }
                bshowusage = false;
            }
            else if (args[i].equals("-o"))    // -o: batch output
            {
                if (i+1 < args.length)
                {
                    szBatchGOoutput = args[i+1];
                }
	        bshowusage = false;
            }
            else if (args[i].equals("-b"))    // -b: batch mode
	    {
	        
                if (i+1 < args.length)
                {
                    szBatchInputDir = args[i+1];
                    if (i+2<args.length)
                    {
                        if (args[i+2].substring(0,1).equals("-")==false)
                        {
                        ...
                        {
                            ;    // not running batch mode if no output file
                        }
                    }
                    else
                    {
	    ...
	    else if (args[i].equals("-i"))  // specifying input dataset for clustering
	    {
	        if (i+1 < args.length)
	        {
	            szDataFileDEF  = args[i+1];
	        }   
	        else
	        {
	            System.out.println("Dataset not specified. Please select the file manually.");
	        }
	    }
	    else if (args[i].equals("-c"))  // Compare model profiles of two datasets
	    {
	        System.out.println("Compare model profiles of two datasets");
	        if (i+2 < args.length)
	        {
	            szDataFileDEF  =args[i+1];
	            szCompareDEF = args[i+2];
	        }   
	        else
	        {
	            System.out.println("Dataset(s) not specified. Please run comparison manually.");
	        }
	    }
            else if (args[i].equals("-C"))
	    {
	        ClusterFlag = true;
	        System.out.println("Compare clusters of two datasets");
	    }
	    else if (args[i].equals("-a"))   // auto mode
	    {
	        autoflag = 1;
	    }
	...
	if (bshowusage)
	{
	    System.out.println("USAGE: java -jar stem.jar [[-d defaultfilename.txt][-o GOoutfile.txt][-c OrigData.tsv CompareData.tsv]|-b batchInput batchOutputDir]");
            return;
	}
